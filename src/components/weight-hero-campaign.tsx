 ...
 * Concept : le jean d'avant, exposé comme une œuvre — une pièce du passé,
 * bientôt de retour. Split-screen sombre, typographie éditoriale.
 */
  ...
  return (
    <section className="relative overflow-hidden bg-[#120e0a] text-[#f3ece2]">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(70% 55% at 68% 30%, rgba(196,120,84,0.16), transparent 65%), radial-gradient(90% 70% at 20% 80%, rgba(0,0,0,0.55), transparent 60%)",
        }}
      />

      <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-6 py-16 lg:grid-cols-12 lg:gap-8 lg:py-24">
        <div className="lg:col-span-6">
          <Reveal>
            <p className="font-mono text-[11px] uppercase tracking-[0.24em] text-clay">
              Weight Management
            </p>
            <h1 className="mt-5 text-balance font-section text-4xl font-medium leading-[1.04] tracking-tight lg:text-6xl">
              {t("Vous le gardez toujours ?", "Do you still keep it?")}
            </h1>
            <p className="mt-6 font-section text-xl italic leading-snug text-[#f3ece2]/85 lg:text-2xl">
              {t("Nous aussi, on connaît ce jean.", "We know those jeans too.")}
            </p>
            <span aria-hidden className="mt-7 block h-px w-12 bg-clay" />
            <p className="mt-6 max-w-[42ch] text-pretty leading-relaxed text-[#f3ece2]/70">
              {t(
            ...
            <div className="mt-9 flex flex-wrap items-center gap-x-8 gap-y-4">
              <Link
                ...
                params={{ slug: "poids" }}
                className="group inline-flex items-center gap-2 rounded-full bg-clay px-7 py-3.5 font-mono text-[11px] uppercase tracking-[0.16em] text-cream transition-all duration-300 hover:brightness-110"
              >
